...
async def get_volume_services(filter_value: str = "", limit: int = 100, detail_level: str = "detailed", **kwargs) -> list[dict]:
    """获取OpenStack卷服务列表，支持过滤和详细程度选项。
    
    Args:
        filter_value: 可选的服务名称或主机过滤器
        limit: 返回结果的最大数量
        detail_level: 返回详细程度 (basic, detailed, full)
        **kwargs: OpenStack连接的额外关键字参数
        
    Returns:
        基于detail_level的卷服务信息字典列表
...
def format_volume_services_summary(services: list[dict], detail_level: str = "detailed") -> str:
    """格式化OpenStack卷服务信息为人类可读的摘要。
    
    Args:
        services: OpenStack卷服务信息列表
    ...
    """
    if not services:
        return "未找到符合条件的OpenStack卷服务。"
    
    # 基本摘要信息
    summary = f"找到 {len(services)} 个OpenStack卷服务:\n\n"
...
async def process_volume_service_query(
    ctx, 
    filter_value: str = "", 
    limit: int = 100, 
    detail_level: str = "detailed",
    get_volume_services_func = None
) -> Optional[list[types.TextContent]]:
    """处理OpenStack卷服务查询的完整流程。
    ...
    Args:
        ...
        limit: 返回结果数量限制
        detail_level: 详细程度
        get_volume_services_func: 获取卷服务的函数
    ...
    try:
        # 异步运行OpenStack查询
        if get_volume_services_func:
            services = await get_volume_services_func(filter_value, limit, detail_level)
        else:
            services = await get_volume_services(filter_value, limit, detail_level)
        
        # 发送成功消息
        await ctx.session.send_log_message(
            level="info",
            data=f"成功获取到 {len(services)} 个OpenStack卷服务",
            logger="openstack",
            related_request_id=ctx.request_id,
        )
        
        # 使用格式化函数生成摘要
        summary = format_volume_services_summary(services, detail_level)
        ...
        # 发送错误信息
        error_message = f"获取OpenStack卷服务信息失败: {str(err)}"
        await ctx.session.send_log_message(
            level="error",
            data=error_message,
            logger="openstack",
